...
public class AutoHDFS extends AbstractAutoCreds {
    ...
    public void doPrepare(Map conf) {
        if(conf.containsKey(HdfsSecurityUtil.STORM_KEYTAB_FILE_KEY) && conf.containsKey(HdfsSecurityUtil.STORM_USER_NAME_KEY)) {
            hdfsKeyTab = (String) conf.get(HdfsSecurityUtil.STORM_KEYTAB_FILE_KEY);
            hdfsPrincipal = (String) conf.get(HdfsSecurityUtil.STORM_USER_NAME_KEY);
        }
    }
    ...
    @SuppressWarnings("unchecked")
    private byte[] getHadoopCredentials(Map conf, Configuration configuration) {
        try {
    ...
    private void login(Configuration configuration) throws IOException {
        if (configuration.get(HdfsSecurityUtil.STORM_KEYTAB_FILE_KEY) == null) {
            configuration.set(HdfsSecurityUtil.STORM_KEYTAB_FILE_KEY, hdfsKeyTab);
        }
        if (configuration.get(HdfsSecurityUtil.STORM_USER_NAME_KEY) == null) {
            configuration.set(HdfsSecurityUtil.STORM_USER_NAME_KEY, hdfsPrincipal);
        }
